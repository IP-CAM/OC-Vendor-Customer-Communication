<?php
class ModelAccountMessageDetail extends Model {
	...
	public function addMessage($data) { 
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "vendor_inquiry_message` SET customer_id='".(int)$this->customer->getId()."', inquiry_id = '" . (int)$data['inquiry_id'] . "', message = '" . $this->db->escape($data['message']) . "',filename = '" . $data['filename'] . "', status = '1', data_added = NOW()");
		...
		$v_subject    	=  !empty($module_laguage[$this->config->get('config_language_id')]['v_message'])?$module_laguage[$this->config->get('config_language_id')]['v_subject']:'';
		$v_message    	=  !empty($module_laguage[$this->config->get('config_language_id')]['v_message'])?$module_laguage[$this->config->get('config_language_id')]['v_message']:'';
		...
		if(!empty($data['inquiry_id'])){
			...
			$subject    = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $v_subject))));
			$message 	= str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $v_message))));
