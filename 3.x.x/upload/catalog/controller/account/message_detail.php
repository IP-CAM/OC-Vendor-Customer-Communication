<?php
class ControllerAccountMessageDetail  extends Controller {
	private $error = array();
	public function index() {
		...
		$this->load->language('account/message_detail');
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('account/message_detail');	
		...
		$this->load->model('account/message_detail');
		$enquiry_info = $this->model_account_message_detail->getEnquiry($inquiry_id);
		...
		$message_show = $this->model_account_message_detail->getTmdMessage($inquiry_id);
		...
		foreach($message_show as $messagesinfo){
			$data['tmdmessage'][]=array(
				...
				'hreflink'     => $this->url->link('account/message_detail/download','&message_id=' . $messagesinfo['message_id']),
		...
		$this->response->setOutput($this->load->view('account/message_detail', $data));
	}
	public function sendDetailMessage() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if(!$json){
				$message_id = $this->model_account_message_detail->addMessage($this->request->post);
		
				$msg_info = $this->model_account_message_detail->getLastMessage($message_id);
				...
				$json['hreflink']     = $this->url->link('account/message_detail/download','&message_id=' . $message_id);
