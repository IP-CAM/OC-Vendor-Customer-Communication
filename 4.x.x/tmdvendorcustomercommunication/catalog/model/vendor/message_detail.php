...
class messagedetail extends \Opencart\System\Engine\Model {
	...
	public function addMessage($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "vendor_inquiry_message` SET vendor_id='".(int)$this->vendor->getId()."', inquiry_id = '" . (int)$data['inquiry_id'] . "', message = '" . $this->db->escape($data['message']) . "',filename = '" . $this->db->escape(serialize($data['filename'])) . "', status = '1', data_added = NOW()");
		...
		$vendor_info  	= $this->model_extension_tmdmultivendor_vendor_vendor->getVendor($enquiry_info['vendor_id']);
